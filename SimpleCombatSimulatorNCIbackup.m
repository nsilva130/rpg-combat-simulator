...
% This latest version keeps track of who won in the file name of a log
% file. It also adds in the option to disable diary logging. Finally, it
% will roll for initiative between the two creatures, breaking ties as
...
c1imod = input('Input first creature''s initiative modifier: '); %|| creature 1 initiative modifier
...
c2imod = input('Input second creature''s initiative modifier: '); %|| creature 2 initiative modifier
...
clog = input('Log combat simulations 1/0 (y/n): ');
...
if init1 > init2 %if creature 1 wins
    t = 1;
    fprintf('%s won the initiative with a %i against %s''s %i. \n',c1name,init1,c2name,init2)
elseif init1 == init2 %if they tie
    if c1imod > c2imod %check modifiers
        t = 1;
        fprintf('%s and %s tied for initiative at %i, but %s won because they have the higher initiative modifier of %i against %s''s %i. \n',c1name,c2name,init1,c1name,c1imod,c2name,c2imod)
    elseif c1imod < c2imod
        t = 2;
        fprintf('%s and %s tied for initiative at %i, but %s won because they have the higher initiative modifier of %i against %s''s %i. \n',c1name,c2name,init2,c2name,c2imod,c1name,c1imod)
    elseif c1imod == c2imod
        t = 1;
        disp('A perfect tie for initiative.')
        fprintf('Defaulting to %s as winner.',c1name) 
    end
elseif init1 < init2 %if creature 2 wins
    t = 2;
    fprintf('%s won the initiative with a %i against %s''s %i . \n',c2name,init2,c1name,init1)
end
...
while tracker == 1
    ...
    fprintf('Round %i. \n',floor(turns/2)) %Display combat time information
    pause(1)
